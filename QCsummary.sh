...
# Description: This script uses FastQC, fqtools, Nozzle.R1 R package and in-house Rscripts to generate report
# with basic quality metrics for every fastq file in a given folder called 'Data_1', for example.
# All raw data is then written to 'Data_1_FastQC_out' folder that is created in the directory that contains a folder with input fastq.gz files('Data_1').
